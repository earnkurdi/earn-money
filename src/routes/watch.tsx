...
declare global {
  interface Window {
    show_11040287?: (opts?: any) => Promise<MonetagResult>;
  }
}
...
async function loadMonetagSdk() {
  ...
  await new Promise<void>((resolve) => {
    ...
    console.log("[Earn][Monetag] loading SDK", {
      zone: MONETAG_ZONE,
    ...
    script.onload = () => {
      ...
      console.log("[Earn][Monetag] SDK loaded", { zone: MONETAG_ZONE });
      resolve();
    };
    script.onerror = () => {
      script.dataset.failed = "true";
      console.error("[Earn][Monetag] SDK failed to load", { zone: MONETAG_ZONE });
      resolve();
    };
  ...
  const reload = useCallback(async () => {
    ...
    const [s, w, h] = await Promise.all([
      supabase
        .from("app_settings")
        .select(
          "reward_per_ad_usd, daily_ad_cap, fallback_offerwall_enabled, fallback_offerwall_name, fallback_offerwall_url",
        )
        .eq("id", 1)
        .maybeSingle(),
      supabase
      ...
      supabase
        .from("ad_watches")
        .select("id, provider, reward_usd, postback_id, created_at")
        .eq("user_id", user.id)
        .order("created_at", { ascending: false })
        .limit(10),
    ]);
    setPerAd(Number(s.data?.reward_per_ad_usd ?? 0));
    setCap(s.data?.daily_ad_cap ?? 50);
    setAdsToday(w.count ?? 0);
  ...
  const watch = async () => {
    ...
    try {
      ...
      if (!sdkReady || typeof window.show_11040287 !== "function") {
        toast.error(t("ad_sdk_unavailable"));
        return;
      }
      const rewardId = createRewardId();
      console.log("[Earn][Monetag] ad requested", {
        zone: MONETAG_ZONE,
        userId: user!.id,
        rewardId,
      });
      ...
      for (const attempt of AD_ATTEMPTS) {
        try {
          ...
          result = await window.show_11040287({
            ...attempt.options,
            ymid: user!.id,
            requestVar: rewardId,
            catchIfNoFeed: true,
          ...
          shown = true;
          console.log("[Earn][Monetag] ad attempt finished", {
            zone: MONETAG_ZONE,
            rewardId,
          ...
          console.warn("[Earn][Monetag] ad attempt failed", {
            zone: MONETAG_ZONE,
            rewardId,
            attempt: attempt.label,
            error,
      ...
      if (!shown) {
        console.warn("[Earn][Monetag] all real ad formats unavailable", {
          zone: MONETAG_ZONE,
          rewardId,
          errors,
        });
        ...
        if (settings?.fallback_offerwall_enabled && settings?.fallback_offerwall_url)
          openOfferwall();
        return;
      }
      if (result?.reward_event_type === "non_valued") {
        toast.error(t("ad_not_paid"));
        setStatus(t("ad_not_paid"));
        return;
      }
      ...
      console.log("[Earn][Monetag] reward credited", {
        rewardId,
        rewardUsd: saved.reward_usd,
        adWatchId: saved.id,
      });
      await reload();
      setStatus("");
      ...
      const safeMessage =
        ...
        message.toLowerCase().includes("no feed") ||
        message.toLowerCase().includes("empty feed") ||
        message.toLowerCase().includes("failed to fetch")
          ? t("ad_inventory_empty")
          : message || t("ad_failed");
    ...
    <AppShell>
      <div className="glass rounded-3xl p-6 text-center">
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          {t("reward_per_ad")}:{" "}
          <span className="text-foreground font-semibold">${perAd.toFixed(4)}</span>
        </p>
        <p className="text-xs text-muted-foreground">
          {t("ads_today")}: {adsToday}/{cap}
        </p>

        <Button
          onClick={watch}
          disabled={busy || adsToday >= cap}
          className="mt-6 w-full glow-primary"
          size="lg"
        >
          {busy ? <RefreshCw className="size-5 animate-spin" /> : <PlayCircle className="size-5" />}{" "}
          {busy ? t("watching") : t("watch_ad")}
        </Button>
            ...
            >
              <div>
                <div className="font-semibold">
                  ${Number(ad.reward_usd).toFixed(4)} · {ad.provider}
                </div>
